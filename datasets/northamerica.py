"""
OEM/northamerica Dataset Loader
"""
import logging
import json
import os
...
num_classes = 8
ignore_label = 255
root = cfg.DATASET.NORTHAMERICA_DIR
img_postfix = '.tif'
...
def make_dataset(mode, maxSkip=0, cv_split=0):
    ...
    logging.info('Northamerica-{}: {} images'.format(mode, len(items) + len(aug_items)))
    return items, aug_items


class NORTHAMERICA(data.Dataset):
...
class NORTHAMERICAUniform(data.Dataset):
    """
    ...
    def __init__(self, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'northamerica_{}_cv{}_tile{}.json'.format(
...
class NORTHAMERICAAug(data.Dataset):
