...
def make_dataset(mode, maxSkip=0, cv_split=0):
    ...
    logging.info('Asia-{}: {} images'.format(mode, len(items) + len(aug_items)))
    return items, aug_items


class ASIA(data.Dataset):
...
class ASIAUniform(data.Dataset):
    """
    ...
    def __init__(self, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'asia_{}_cv{}_tile{}.json'.format(
